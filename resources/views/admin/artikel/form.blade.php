...
<div class="form-container">
    <h1><i class="fas fa-newspaper" style="color:#b45309"></i> {{ $editing ? 'Edit Artikel' : 'Tambah Artikel' }}</h1>
    <form method="POST" action="{{ $formAction }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input id="judul" name="judul" required value="{{ old('judul', $artikel?->judul) }}">
        </div>
        <div class="form-group">
            <label for="kategori">Kategori</label>
            <select id="kategori" name="kategori" required>
                @foreach(['Tips', 'Wisata', 'Event', 'Kuliner', 'Lainnya'] as $kategori)
                    <option value="{{ $kategori }}" @selected(old('kategori', $artikel?->kategori ?? 'Tips') === $kategori)>{{ $kategori }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="penulis">Penulis</label>
            <input id="penulis" name="penulis" required value="{{ old('penulis', $artikel?->penulis ?? 'Admin Kos XYZ') }}">
        </div>
        <div class="form-group">
            <label for="tanggal_publikasi">Tanggal Publikasi</label>
            <input type="date" id="tanggal_publikasi" name="tanggal_publikasi" required value="{{ old('tanggal_publikasi', $artikel?->tanggal_publikasi?->format('Y-m-d') ?? now()->format('Y-m-d')) }}">
        </div>
        <div class="form-group">
            <label for="deskripsi_singkat">Deskripsi Singkat</label>
            <textarea id="deskripsi_singkat" name="deskripsi_singkat" rows="3">{{ old('deskripsi_singkat', $artikel?->deskripsi_singkat) }}</textarea>
        </div>
        <div class="form-group">
            <label for="isi">Isi Artikel</label>
            <textarea id="isi" name="isi" required>{{ old('isi', $artikel?->isi) }}</textarea>
            <div class="helper">HTML sederhana seperti &lt;p&gt;, &lt;strong&gt;, &lt;ul&gt; diperbolehkan.</div>
        </div>
        <div class="form-group">
            <label for="gambar">Foto Artikel</label>
            @if($artikel?->gambar)
                <img class="preview" src="{{ $artikel->gambar_url }}" alt="{{ $artikel->judul }}">
            @endif
        ...
        <div class="form-group">
            <label><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $artikel?->is_active ?? true))> Tampilkan di landing page</label>
        </div>
        <div class="actions">
            <button class="save" type="submit"><i class="fas fa-save"></i> {{ $submitLabel }}</button>
            <a class="cancel" href="{{ route('admin.artikel.index') }}">Batal</a>
        </div>
    </form>
</div>
